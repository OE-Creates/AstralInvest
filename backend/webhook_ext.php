<?php
    ...
    function SendWebhookSuccessEmail($paymentid, $chargevendor)
    {
        ...
        $getdata = "SELECT payment_descriptor, payment_name, payment_email, charge_time, charge_amount FROM transactions WHERE payment_id = '$paymentid'";
    ...
    function SendWebhookRefundEmail($paymentid)
    {
        ...
        $updatepayrefunded = "UPDATE transactions SET refund_processed = 1 WHERE payment_id = '$paymentid'";
        $runupdatepayrefunded = $conn->query($updatepayrefunded);

        $getdata = "SELECT payment_descriptor, customer_id, payment_name, payment_email, charge_time, charge_amount, refund_time FROM transactions WHERE payment_id = '$paymentid'";
        $get = $conn->query($getdata);
        $row = $get->fetch_array();
    ...
    function SetReferralDataIfAny($paymentid)
    {
        ...
        $getdata = "SELECT customer_id, charge_amount FROM transactions WHERE payment_id = '$paymentid' AND payment_descriptor LIKE '%Contribution Plan%'";
        $get = $conn->query($getdata);
