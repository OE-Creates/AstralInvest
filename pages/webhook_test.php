<?php
	...
	include '../backend/webhook_ext_test.php';
	...
	if ($_SERVER["REQUEST_METHOD"] === "POST")
	{
		...
		foreach ($headers as $header => $value)
		{
			if ($header == "event-type" && $value == "payment.payment.create")
			{
				...
				$checkpaymentid = "SELECT payment_id FROM transactions_test WHERE payment_id = '$paymentid'";
				$check = $conn->query($checkpaymentid);

				if ($check->num_rows === 0)
				{
					$addnewdata = "INSERT INTO transactions_test (id, payment_id, payment_currency, payment_time, payment_descriptor, payment_disputed, customer_id, payment_name, payment_email, cid, charge_nooffarms, rid)
				...
				{
					$updnewdata = "UPDATE transactions_test SET id = '$id', payment_currency = '$paymentcurrency', payment_time = '$paymenttime', payment_descriptor = '$paymentdescriptor', payment_disputed = '$paymentdisputed',
			...
			if ($header == "event-type" && $value == "payment.charge.create")
			{
				...
				if ($check->num_rows !== 0)
				{
					$addchargedata = "UPDATE transactions_test SET cid = '$cid', charge_time = '$chargetime', charge_recid = '$chargerecid', charge_tokentype = '$chargetokentype', charge_source = '$chargesource', charge_result = '$chargeresult',
			...
			if ($header == "event-type" && $value == "payment.charge.update")
			{
				...
				if ($check->num_rows !== 0)
				{
					$updatechargedata = "UPDATE transactions_test SET cid = '$cid', charge_vendor = '$chargevendor', charge_result = '$chargeresult', charge_responsecode = '$chargeresponsecode',
			...
			if ($header == "event-type" && $value == "payment.refund.create")
			{
				...
				if ($check->num_rows !== 0)
				{
					$updrefunddata = "UPDATE transactions_test SET rid = '$rid', refund_time = '$refundtime', refund_reason = '$refundreason', charge_result = '$refundresult', charge_responsecode = '$refundresponsecode',
